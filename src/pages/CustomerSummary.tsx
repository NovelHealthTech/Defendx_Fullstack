...
export default function CustomerSummary() {
				...
				if (json.success && json.data) {
					...
					setCustomer({
						...
						contractEndDate: String(
							data.attributes?.["Contract end date"] ?? "-"
						),
						internalOwner: String(
							data.attributes?.["Internal owner"] ?? "-"
						),
						domain_count_active: Number(
							data.domain_count_active ?? 0
						),
					});
					setRiskBreakdown([
						{
							label: "Critical",
							value: Number(
								data.overall_risk_counts.critical ?? 0
							),
							color: "#dc2626",
						},
						{
							label: "High",
							value: Number(data.overall_risk_counts.high ?? 0),
							color: "#f59e42",
						},
						{
							label: "Medium",
							value: Number(data.overall_risk_counts.medium ?? 0),
							color: "#fbbf24",
						},
						{
							label: "Low",
							value: Number(data.overall_risk_counts.low ?? 0),
							color: "#22c55e",
						},
					]);
					// setPieData([
					...
					// 		value: Number(
					// 			data.categoryScores.ipDomainReputation ?? 0
					// 		),
					// 		color: "#22c55e",
					// 	},
					// 	{
					// 		name: "Encryption",
					// 		value: Number(data.categoryScores.encryption ?? 0),
					// 		color: "#fbbf24",
					// 	},
					// 	{
					// 		name: "Vulnerability Management",
					// 		value: Number(
					// 			data.categoryScores.vulnerabilityManagement ?? 0
					// 		),
					// 		color: "#22c55e",
					// 	},
					// 	{
					// 		name: "Attack Surface",
					// 		value: Number(
					// 			data.categoryScores.attackSurface ?? 0
					// 		),
					// 		color: "#22c55e",
					// 	},
					// ]);
	...
	const riskManagement = [
		...
		{ title: "Additional Evidence", uploaded: 0, shared: 0, requested: 0 },
		{
			...
			active: customer?.domain_count_active ?? 0,
		},
		{
	...
	function ScoreByEpochSection() {
		...
		const metrics = Object.keys(customer.scores_by_epoch[0] || {}).filter(
			(k) => !["epoch", "when", "overall"].includes(k)
		);
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						...
						<span
							...
							title="Compare security scores for different time periods across multiple metrics. Toggle lines to focus on specific epochs."
						>
							<Info className="w-4 h-4 text-muted-foreground" />
						</span>
					</CardTitle>
					<div className="flex flex-wrap gap-2 mt-2">
						{customer.scores_by_epoch.map(
							(epochObj: any, idx: number) => (
								<label
									key={epochObj.epoch}
								...
								>
									<input
										type="checkbox"
										checked={visibleEpochs.includes(
											epochObj.epoch
										)}
										onChange={() =>
											handleEpochToggle(epochObj.epoch)
										}
									/>
									<span
										style={{
											color: epochColors[
												idx % epochColors.length
											],
										}}
									>
										{epochObj.epoch}
				...
				<CardContent>
					...
					<div className="mt-3 flex items-start gap-2 text-sm text-muted-foreground">
						...
						<span>
							<b>Score by Epoch</b> shows how your security scores
							for different metrics (like website, email, network,
							etc.) have changed over time. Toggle epochs above to
							compare or focus on specific periods.
						</span>
					</div>
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						...
						<span
							className="ml-2 cursor-pointer"
							title="See the number of risks by severity for each security category. Toggle severities to focus on specific risk levels."
						>
							<Info className="w-4 h-4 text-muted-foreground" />
						</span>
					</CardTitle>
						...
						{["Critical", "High", "Medium", "Low"].map(
							(severity) => (
								...
								>
									<input
										type="checkbox"
										checked={visibleSeverities.includes(
											severity
										)}
										onChange={() =>
				...
				<CardContent>
					...
					<div className="mt-3 flex items-start gap-2 text-sm text-muted-foreground">
						<Info className="w-4 h-4 mt-0.5" />
						<span>
							<b>Risk Ratings Overview</b> displays the count of
							risks by severity (Critical, High, Medium, Low) for
							each security category. Toggle severities above to
							compare or focus on specific risk levels.
						</span>
					</div>
				</CardContent>
			</Card>
